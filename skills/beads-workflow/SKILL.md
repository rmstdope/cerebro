...
| **awaiting the navigator's merge** | `human`, a `## Ready to merge` note naming the pull request; `planned` and the stage label kept | a producer or the bugfixer under the `merge` gate (`produce-bead`, *The navigator's gates*) | the navigator: merge on GitHub and `bd close`; or unpark it and the next producer resumes from the design and finishes |
