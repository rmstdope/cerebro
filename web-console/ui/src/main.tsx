...
function App() {
  ...
  const refresh = async () => { setRefreshing(true); const [f, w] = await Promise.allSettled([read<Agent[]>("/api/fleet"), read<Work>("/api/work")]); if (f.status === "fulfilled") { setFleet(f.value); if (f.value.state === "fresh") setUpdated(Date.now()); } else setFleet(previous => previous && previous.state !== "unavailable" ? { state: "stale", value: previous.value, error: f.reason.message } : { state: "unavailable", error: f.reason.message }); if (w.status === "fulfilled") { setWork(w.value); if (w.value.state === "fresh") setUpdated(Date.now()); } else setWork(previous => previous && previous.state !== "unavailable" ? { state: "stale", value: previous.value, error: w.reason.message } : { state: "unavailable", error: w.reason.message }); setRefreshing(false); };
  ...
  const updatedAge = updated ? age(new Date(updated).toISOString()) : "—";
  return <main><header className="masthead"><div><h1>Cerebro</h1><p>Fleet and work · read-only</p></div><div><p>Updated {updatedAge}</p><button onClick={() => void refresh()} disabled={refreshing}>{refreshing ? "Refreshing…" : "Refresh"}</button></div></header>
    ...
    {stale && <p className="stale">Showing information from {updatedAge}. Refresh to try again.</p>}
