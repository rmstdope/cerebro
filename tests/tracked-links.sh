...
fix="$(new_fixture)"
git_q -C "$fix" rm -q --cached .github/skills/produce-bead >/dev/null
rm "$fix/.github/skills/produce-bead"
...
grep -qF "missing: .github/skills/produce-bead (skills/produce-bead is shipped and has no tracked link)" <<<"$out" \
