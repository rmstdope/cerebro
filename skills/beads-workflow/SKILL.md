...
| **waiting on the navigator** | open, unassigned, `human`, `planned` removed | any role escalating; `scripts/producer-park … scope`; a `ux` agent parking | nobody: `bd human list` and the fleet view's *Waiting on you*. Every candidate script excludes `human`. The navigator (or Cerebro's unpark sweep) removes it |
