...
| One planner owns a whole split family, marked `planner:<name>` on the parent — retired by cb-10d.2.2 | ah-ywj7 |
| A hold names its holder (`planning:<name>`) so a finishing session cannot strip another's — retired by cb-10d.2.2 | ah-ywj7 |
