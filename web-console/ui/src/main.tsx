...
function Dialog({ item, onClose }: { item: Agent | Bead; onClose: () => void }) {
  ...
  useEffect(() => { close.current?.focus(); const key = (event: KeyboardEvent) => event.key === "Escape" && onClose(); addEventListener("keydown", key); return () => removeEventListener("keydown", key); }, [onClose]);
  const agent = "role" in item;
  return <div className="scrim" onMouseDown={onClose}><section className="dialog" role="dialog" aria-modal="true" aria-label="Read-only details" onMouseDown={event => event.stopPropagation()}>
    <header><div><h2>{agent ? item.name : item.title}</h2><p>Read-only details</p></div><button ref={close} onClick={onClose}>Close</button></header>
...
function App() {
  ...
  const [selected, setSelected] = useState<Agent | Bead>();
  const origin = useRef<HTMLElement | null>(null);
  const refresh = async () => { setRefreshing(true); const [f, w] = await Promise.allSettled([read<Agent[]>("/api/fleet"), read<Work>("/api/work")]); if (f.status === "fulfilled") setFleet(f.value); else setFleet(previous => previous?.state === "fresh" ? { state: "stale", value: previous.value, error: f.reason.message } : previous); if (w.status === "fulfilled") setWork(w.value); else setWork(previous => previous?.state === "fresh" ? { state: "stale", value: previous.value, error: w.reason.message } : previous); setRefreshing(false); };
  useEffect(() => { void refresh(); const timer = setInterval(() => void refresh(), 15000); return () => clearInterval(timer); }, []);
  const choose = (item: Agent | Bead, event: React.MouseEvent<HTMLButtonElement>) => { origin.current = event.currentTarget; setSelected(item); };
  const close = () => { setSelected(undefined); origin.current?.focus(); };
  const stale = fleet?.state === "stale" || work?.state === "stale";
  const lanes: [string, Bead[]][] = work?.state !== "unavailable" ? [["Backlog", [...work!.value.unplanned, ...work!.value.paused]], ["Designing UX", work!.value.being_planned], ["UX designed", [...work!.value.ux_agreed, ...work!.value.planned]], ["In progress", work!.value.claimed], ["Unverified", work!.value.merged]] : [];
  ...
  return <main><header className="masthead"><div><h1>Cerebro</h1><p>Fleet and work · read-only</p></div><div><p>Updated {stale ? "previously" : "just now"}</p><button onClick={() => void refresh()} disabled={refreshing}>{refreshing ? "Refreshing…" : "Refresh"}</button></div></header>
    ...
    {stale && <p className="stale">Showing information from 2 minutes ago. Refresh to try again.</p>}
    ...
    {selected && <Dialog item={selected} onClose={close} />}
  </main>;
}
