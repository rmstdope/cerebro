...
- **Cerebro** (`orchestrator`, Opus/medium) — stops implementers on request by writing their stop
  ...
  look at the work the view kept, which needs a judgement no decision table makes. The
  ...
  sweep looks for and the guards it runs under); what is left for a Cerebro session is the work
  the view kept rather than throw away, and the beads parked on the navigator, each of which
...
- **`scripts/assignable-beads` is the one place "which beads may an implementer be given" is
  ...
  Since cb-10d.4 `release-bead --ended <Name> <id>` takes back what a gone session still holds: it
  clears an open assignment, closes an `in_progress` claim whose work `scripts/bead-delivery.sh` finds
  on main, and otherwise answers `kept <reason>` for Cerebro. Its push is no longer advisory.
- **`scripts/bead-delivery.sh` is the one place bash answers "did this bead's work reach the default
  branch"** (cb-10d.4) — `cerebro_bead_delivered <root> <default branch> <id>`, sourced never
  executed, working under `set -e` and without it. It reads `commit_ref_pattern` (default `({id}):`,
  substituted rather than prefixed) and `non_delivery_commit_pattern` (default empty) through
  `project-conf`, and assumes its caller fetched. Its one caller is `release-bead --ended`, and its
  suite is `tests/bead-delivery.sh`; it was extracted unchanged from the retired claims sweep.
