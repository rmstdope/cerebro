...
| **UX-agreed** | `ux:agreed`, unassigned | the `ux` agent (`agree-experience`) | a producer, through `scripts/assignable-beads`, claimed for it by the fleet view. An unclaimed bead that also carries `planned` is a producer's returned plan (rework, or a session that died) and is resumed from its `design` |
...
| **waiting on the navigator** | open, unassigned, `human`, `planned` removed | any role escalating; `scripts/producer-park … scope` (`human` alone, never `needs-ui-decision`); a `ux` agent parking | nobody: `bd human list` and the fleet view's *Waiting on you*. Every candidate script excludes `human`. The navigator, or Cerebro's unpark, removes `human` and `pause:kept` only |
