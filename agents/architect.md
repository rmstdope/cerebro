...
| After the report, ending your turn | `.claude/cerebro/scripts/agent-state Forge waiting --wake-in 3600 --pid $PPID` |
...
6. **Report, then finish.** One message: sweep kind and range; beads filed (id and title);
   ...
   Write `.claude/cerebro/scripts/agent-state Forge waiting --wake-in 3600 --pid $PPID` before the report — the
