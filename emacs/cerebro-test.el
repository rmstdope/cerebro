...
;; --- templates/consumer-dir-locals.el ------------------------------------
;;
;; The file a consumer gets at its root from `scripts/sync-symlinks.sh'. It is data, not a
;; script: Emacs `read's it and takes the FIRST form as an alist of (MODE . ((VAR . VALUE)...)).
;; A file of bare `add-to-list'/`require' calls - the shape anyone writes first - is read as
;; settings for a major mode named `add-to-list', applies nothing, and fails in silence. Hence a
;; test on the structure, and a second one that actually runs the form.

(defconst cerebro-test--dir-locals-template
  (expand-file-name "templates/consumer-dir-locals.el" cerebro-test--repo-root))

(defun cerebro-test--read-dir-locals (file)
  "The first form in FILE, the way `dir-locals-read-from-dir' reads one."
  (with-temp-buffer
    (insert-file-contents file)
    (goto-char (point-min))
    (read (current-buffer))))

(ert-deftest cerebro-test/dir-locals-template-is-an-alist-not-a-script ()
  (let ((form (cerebro-test--read-dir-locals cerebro-test--dir-locals-template)))
    (should (consp form))
    ;; Every entry keys on a mode symbol or nil, and carries an alist of settings.
    (dolist (entry form)
      (should (consp entry))
      (should (or (null (car entry)) (symbolp (car entry))))
      (should (listp (cdr entry))))
    (should (assq nil form))
    (should (assq 'eval (alist-get nil form)))))

(ert-deftest cerebro-test/dir-locals-template-autoloads-cerebro-from-the-mount ()
  "Running the form from a subdirectory finds the mount and registers the command."
  (let* ((root (file-name-as-directory (make-temp-file "cerebro-dir-locals-" t)))
         (deep (expand-file-name "a/b/" root))
         (mount (expand-file-name ".claude/cerebro" root))
         (form (alist-get 'eval (alist-get nil (cerebro-test--read-dir-locals
                                                cerebro-test--dir-locals-template)))))
    (unwind-protect
        (progn
          (make-directory deep t)
          (make-directory (file-name-directory mount) t)
          (make-symbolic-link cerebro-test--repo-root mount)
          ;; `load-path' is global and the autoload would redefine a command this very
          ;; session has loaded, so both are restored.
          (let ((load-path (copy-sequence load-path))
                (real (symbol-function 'cerebro)))
            (unwind-protect
                (progn
                  (fmakunbound 'cerebro)
                  (let ((default-directory deep))
                    (eval form t))
                  (should (member (expand-file-name ".claude/cerebro/emacs" root) load-path))
                  (should (commandp 'cerebro)))
              (fset 'cerebro real))))
      (delete-directory root t))))

(ert-deftest cerebro-test/dir-locals-template-is-inert-without-a-mount ()
  "A project file open where no .claude/cerebro is above it must not error."
  (let* ((root (file-name-as-directory (make-temp-file "cerebro-no-mount-" t)))
         (form (alist-get 'eval (alist-get nil (cerebro-test--read-dir-locals
                                                cerebro-test--dir-locals-template)))))
    (unwind-protect
        (let ((load-path (copy-sequence load-path))
              (default-directory root))
          (should-not (eval form t)))
      (delete-directory root t))))
